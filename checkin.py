...
class GLaDOS:
    ...
    def exchange(self, plan):
        """执行兑换"""
        required = EXCHANGE_PLANS.get(plan, 500)
        pts = int(self.points) if self.points != "?" else 0
        if pts < required:
            self.exchange_result = f"积分不足({pts}/{required})"
            return False

        res = self.req('POST', '/api/user/exchange', {'planType': plan})
        if res:
            code = res.get('code', -2)
...
def main():
    ...
    for i, cookie in enumerate(cookies, 1):
        ...
        if "兑换成功" in g.exchange_result: exchange_cnt += 1
